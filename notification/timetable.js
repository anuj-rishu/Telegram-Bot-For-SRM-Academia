...
class NotificationService {
    ...
    async checkUpcomingClasses(userId) {
        ...
        try {
            const calendarResponse = await apiService.makeAuthenticatedRequest("/calendar", session);
            if (calendarResponse.data.today.dayOrder === "-") return;

            const response = await apiService.makeAuthenticatedRequest("/timetable", session);
            const todaySchedule = response.data.schedule.find(
                day => day.day === parseInt(calendarResponse.data.today.dayOrder)
            );
            ...
            todaySchedule.table.forEach(async (slot) => {
                if (!slot) return;
            ...
            console.error(`❌ Error checking classes for user ${userId}:`, error.message);
        }
    }
    ...
    async sendClassReminder(userId, slot) {
        ...
        try {
            ...
            console.error(`❌ Failed to send class reminder to user ${userId}:`, error.message);
        }
    }
    ...
    async sendDailySchedule(userId) {
        ...
        const context = {
            ...
            replyWithMarkdown: (text) => this.bot.telegram.sendMessage(userId, text, { 
                parse_mode: 'Markdown',
                disable_web_page_preview: true
            })
        };
