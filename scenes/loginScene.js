...
const loginScene = new Scenes.WizardScene(
  ...
  async (ctx) => {
    try {
      ...
      const [user, inactiveUser] = await Promise.all([
        ...
        InactiveUser.findOne({ telegramId: userId }),
      ]);
  ...
  async (ctx) => {
    ...
    try {
      ...
      try {
        const [testResponse, userResponse, marksResponse, attendanceResponse] =
          await Promise.all([
        ...
        if (error.code === "ETIMEDOUT" || error.message.includes("timeout")) {
          await ctx.reply("❌ Login timed out. Please try again later.");
        } else if (
