...
const loginScene = new Scenes.WizardScene(
  ...
  async (ctx) => {
    try {
      ...
      ctx.wizard.state.isReactivation = Boolean(
        await InactiveUser.findOne({ telegramId: userId })
      );
  ...
  async (ctx) => {
    ...
    try {
      ...
      if (
        ...
        isErrorMessage(loginResponse.data?.message)
      ) {
        await ctx.reply(
      ...
      try {
        ...
        if (!testResponse.data || testResponse.data.error) {
          await ctx.telegram
            .deleteMessage(ctx.chat.id, fetchingMsg.message_id)
            .catch(() => {});
          await ctx.reply(
        ...
        if (ctx.wizard.state.isRelogin) {
          ...
          await ctx.reply(
            "All your previous data and settings have been restored. You can continue using the bot commands from Menu ≡"
          );
        } else {
          await ctx.reply(
...
loginScene.action("agree_privacy", async (ctx) => {
  try {
    ...
    if (ctx.wizard.state.isRelogin) {
      ...
      welcomeText =
        "Welcome back! Please enter your SRM username/email to reactivate your account:";
    }
...
loginScene.action("disagree_privacy", async (ctx) => {
  try {
    await ctx.editMessageText(
      "You must agree to the Privacy Policy to use this bot."
    );
    return ctx.scene.leave();
  } catch (error) {
